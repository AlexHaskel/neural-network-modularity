...
def plot_eigengaps(EIG_GAPS, folder, file_name, file_format = 'pdf', figsize = (4,3), max_gap=5, **kwargs):

	plt.figure(figsize = figsize)

	for i, gaps in enumerate(EIG_GAPS):

	    color = mpl.colormaps.get_cmap('plasma')(i/len(EIG_GAPS))
	    gap_indices = np.arange(1, len(gaps) + 1) 
	    mask = gap_indices <= max_gap	    

	    plt.scatter(gap_indices[mask], gaps[mask], color = color, s = 10)
	    plt.plot(gap_indices[mask], gaps[mask], color = color, zorder = -10, label = f'layer {i}')

	plt.legend()
	plt.xlabel('eigengap index')
	plt.ylabel('eigengap magnitude')
	plt.xlim(0.5, max_gap + 0.5)
	plt.xticks(np.arange(1, max_gap + 1))
	plt.ylim(-0.2,1.2)
	plt.tight_layout()
	plt.savefig(f'{folder}/{file_name}.{file_format}', **kwargs)
	plt.close()
